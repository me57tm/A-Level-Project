...
time = [start timer]
while time < 30:
    [wait until submit button pressed]
    maxScore = maxScore + 1
question = module.generateQuestion()
correctAnswer = module.answerQuestion(question)
userAnswer = input(question)
if userAnswer == correctAnswer then
        score = score + 1
    endif
    if [home button pressed] then
        [go to homescreen]
    endif
endwhile[add to database score and maxScore]
...
number = random_int(2,5)
exponents = []
coefficients = []
for i = 1 to number
    append to exponents generateNumber()
    append to coefficients generateNumber()
next i
[output the maths in a way that makes sense]
*/
...
function generateQuestion(){
	...
	return ["Integrate:<br>y = @",substr($tempString,0,sizeof($tempString)-2)];
}
...
function check($q,$givenA){
	...
	echo $a . '<br>';
	echo sortOutNonsense($givenA[0],$givenA[1]) . "<br><br>";
